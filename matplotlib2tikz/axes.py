...
class Axes(object):
    def __init__(self, data, obj):
        ...
        self.axis_options.append(("xmin={:"+data['float_format'].strip('%')+"}, xmax={:"+data['float_format'].strip('%')+"}").format(*xlim))
        ylim = sorted(list(obj.get_ylim()))
        self.axis_options.append(("ymin={:"+data['float_format'].strip('%')+"}, ymax={:"+data['float_format'].strip('%')+"}").format(*ylim))
    ...
    def _colorbar(self, colorbar, data):
        ...
        self.axis_options.append(("point meta min="+data['float_format']+"") % limits[0])
        self.axis_options.append(("point meta max="+data['float_format']+"") % limits[1])
...
def _get_ticks(data, xy, ticks, ticklabels):
    ...
    if data["strict"] or is_label_required:
        if pgfplots_ticks:
            axis_options.append(
                "%stick={%s}" % (xy, ",".join([(""+data['float_format']+"") % el for el in pgfplots_ticks]))
            )
        else:
...
def _handle_linear_segmented_color_map(cmap,data):
    ...
    for (k, x) in enumerate(X):
        color_changes.append(("rgb(%d%s)=("+data['float_format']+","+data['float_format']+","+data['float_format']+")") % ((x, unit) + colors[k]))
...
def _handle_listed_color_map(cmap,data):
    ...
    if cmap.N is None or cmap.N == len(cmap.colors):
        colors = [
            ("rgb(%d%s)=("+data['float_format']+","+data['float_format']+","+data['float_format']+")") % (k, unit, rgb[0], rgb[1], rgb[2])
